...
func (c *Client) Connect() error {
	...
	if err != nil {
		log.Error("connect err:", zap.Any("session", c.session), zap.Error(err))
		return err
	}
	log.Info(" url, connected", zap.Any("session", c.session), zap.Any("url", c.session.URL))
...
func (c *Client) Listening() error {
	...
	for {
		...
		case <-resumeSignal: // 使用信号量控制连接立即重连
			log.Info(" received resumeSignal signal", zap.Any("session", c.session))
			return errs.ErrNeedReConnect
		case err := <-c.closeChan:
			// 关闭连接的错误码 https://bot.q.qq.com/wiki/develop/api/gateway/error/error.html
			log.Error("Listening stop. err is ", zap.Any("session", c.session), zap.Error(err))
			// 不能够 identify 的错误
		...
		case <-c.heartBeatTicker.C:
			log.Info("listened heartBeat", zap.Any("session", c.session))
...
func (c *Client) Write(message *dto.WSPayload) error {
	...
	log.Info(" write  message,", zap.Any("session", c.session), zap.Any("OPMeans", dto.OPMeans(message.OPCode)), zap.Any("m", string(m)))

	if err := c.conn.WriteMessage(wss.TextMessage, m); err != nil {
		log.Error(" WriteMessage failed", zap.Any("session", c.session), zap.Error(err))
		c.closeChan <- err
		return err
	}
	return nil
...
func (c *Client) Close() {
	if err := c.conn.Close(); err != nil {
		log.Error("close conn err: ", zap.Any("session", c.session), zap.Error(err))
	}
...
func (c *Client) readMessageToQueue() {
	for {
		...
		if err != nil {
			log.Error(" read message failed, , message ", zap.Any("session", c.session), zap.Error(err), zap.Any("message", string(message)))
		...
		if err := json.Unmarshal(message, payload); err != nil {
			log.Error(" json failed, ", zap.Any("session", c.session), zap.Error(err))
			continue
		}
		payload.RawMessage = message
		log.Info("receive  message, ", zap.Any("session", c.session), zap.Any("OPCode", dto.OPMeans(payload.OPCode)), zap.Any("message", string(message)))
...
func (c *Client) listenMessageAndHandle() {
	...
	for payload := range c.messageQueue {
		...
		if err := event.ParseAndHandle(payload); err != nil {
			log.Error(" parseAndHandle failed, ", zap.Any("session", c.session), zap.Error(err))
		}
	}
	log.Info(" message queue is closed", zap.Any("session", c.session))
}
...
func (c *Client) startHeartBeatTicker(message []byte) {
	...
	if err := event.ParseData(message, helloData); err != nil {
		log.Error(" hello data parse failed, , message ", zap.Any("session", c.session), zap.Error(err), zap.Any("message", string(message)))
	}
...
func (c *Client) readyHandler(payload *dto.WSPayload) {
	...
	if err := event.ParseData(payload.RawMessage, readyData); err != nil {
		log.Error(" parseReadyData failed, message ", zap.Any("session", c.session), zap.Error(err), zap.Any("RawMessage", string(payload.RawMessage)))
	}
